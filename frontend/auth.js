...
const BILLING_CACHE_KEY_PREFIX = "billingDataCacheV4";
const BILLING_CACHE_TTL_MS = 120000;
const FIRST_USE_BILLING_ACK_PREFIX = "statementReviewFirstUseBillingAck";
...
function billingMonthFromData(data) {
  return String((data && data.report && data.report.month) || new Date().toISOString().slice(0, 7).replace("-", ""));
}
...
function acknowledgeFirstUseBilling(userId, data) {
  try {
    ...
    // Ignore storage errors; the server-side billing checks still apply.
  }
}
...
function showFirstUseBillingDialog(data, user) {
  ...
  body.textContent =
    "This is the first Statement Review for " + poolLabel + " this month. " +
    "A monthly platform cost of " + monthlyCost + " starts being allocated on first successful use. " +
    "This is the website operating cost divided across active documents in your organization, so your estimated share can change as other users process statements.";
